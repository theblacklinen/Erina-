...
from ErinaBot import telethn as tbot, dispatcher
from ErinaBot.events import register
from ErinaBot.modules.sql.nightmode_sql import (
    add_nightmode,
    rmnightmode,
    get_all_chat_id,
    is_nightmode_indb,
        ...
        try:
            ...
            await tbot(
                functions.messages.EditChatDefaultBannedRightsRequest(
                    peer=int(warner.chat_id), banned_rights=SungJin
                )
            )
...
async def job_open():
    ...
    for warner in ws_chats:
        try:
            ...
            await tbot(
                functions.messages.EditChatDefaultBannedRightsRequest(
                    peer=int(warner.chat_id), banned_rights=openSungJin
                )
            )
        except Exception as e:
