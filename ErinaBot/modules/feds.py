...
import ErinaBot.modules.sql.feds_sql as sql
from ErinaBot import (EVENT_LOGS, LOGGER, OWNER_ID, DRAGONS, TIGERS, WOLVES,
                          dispatcher)
from ErinaBot.modules.disable import DisableAbleCommandHandler
from ErinaBot.modules.helper_funcs.alternate import send_message
from ErinaBot.modules.helper_funcs.chat_status import is_user_admin
from ErinaBot.modules.helper_funcs.extraction import (extract_unt_fedban,
                                                          extract_user,
                                                          extract_user_fban)
from ErinaBot.modules.helper_funcs.string_handling import markdown_parser
...
def new_fed(update: Update, context: CallbackContext):
    ...
    if fednam != '':
        ...
        if not x:
            update.effective_message.reply_text(
                "Can't federate! Please contact @xelcius if the problem persist."
            )
            return
...
def fed_ban(update: Update, context: CallbackContext):
    ...
    if fban:
        ...
        if not x:
            message.reply_text(
                "Failed to ban from the federation! If this problem continues, contact @xelcius."
            )
            return
    ...
    if args:
        if args[0] == 'json':
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "SungJinWoo_fbanned_users.json"
                update.effective_message.reply_document(
                    document=output,
                    filename="SungJinWoo_fbanned_users.json",
