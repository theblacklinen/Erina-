...
from ErinaBot.modules.helper_funcs.chat_status import (
    dev_plus,
    sudo_plus,
    whitelist_plus,
)
from ErinaBot.modules.helper_funcs.extraction import extract_user
from ErinaBot.modules.log_channel import gloggable
...
def addsudo(update: Update, context: CallbackContext) -> str:
    ...
    if user_id in DRAGONS:
        message.reply_text("Already a First Seat")
        return ""
    ...
    update.effective_message.reply_text(
        (
            rt
            + f"\nSuccessfully licenced {user_member.first_name} as a First Seat"
        )
    )
...
def addsupport(
    ...
    if user_id in DRAGONS:
        rt += "You are no more a First Seat"
...
def addwhitelist(update: Update, context: CallbackContext) -> str:
    ...
    if user_id in DRAGONS:
        rt += "This member is a First Seat, Lets Make Him Second Seat"
        data["sudos"].remove(user_id)
        DRAGONS.remove(user_id)

    if user_id in DEMONS:
        rt += "You are our friend, but it's for your own good if you become a Second Seat instead."
        data["supports"].remove(user_id)
        DEMONS.remove(user_id)

    if user_id in WOLVES:
        message.reply_text("This user is already a true Second Seat")
        return ""
    ...
    update.effective_message.reply_text(
        f"{rt}\nSuccessfully promoted {user_member.first_name} to a ranked Second Seat!"
    )
...
def addtiger(update: Update, context: CallbackContext) -> str:
    ...
    if user_id in DRAGONS:
        rt += "You were a First Seat, but now you are just a classmate"
        data["sudos"].remove(user_id)
        DRAGONS.remove(user_id)
    ...
    if user_id in WOLVES:
        rt += "This user is already a Second Seat, we can be classmates as well.."
        data["whitelists"].remove(user_id)
        WOLVES.remove(user_id)
...
def removesudo(update: Update, context: CallbackContext) -> str:
    ...
    if user_id in DRAGONS:
        message.reply_text("You are not  First Seat anymore!")
        DRAGONS.remove(user_id)
        data["sudos"].remove(user_id)
        ...
        return log_message
    message.reply_text("This user is not an First Seat!")
    return ""
...
def removewhitelist(update: Update, context: CallbackContext) -> str:
    ...
    if user_id in WOLVES:
        ...
        return log_message
    message.reply_text("This user is not a Second Seat!")
    return ""
