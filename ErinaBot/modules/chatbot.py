...
import html
import requests
import ErinaBot.modules.sql.kuki_sql as sql
...
def kuki_message(context: CallbackContext, message):
    ...
    if message.text.lower() == "SungJinWoo":
        return True
    if reply_message:
...
def chatbot(update: Update, context: CallbackContext):
    ...
    if message.text and not message.document:
        ...
        kukiurl = requests.get(
            f'https://kukiapi.xyz/api/apikey=5281955434-KUKIyq4NCB2Ca8/SungJinWoo/@xelcius/message={Message}'
        )
...
__help__ = """
Chatbot utilizes the Kuki's API which allows SungJinWoo to talk and provide a more interactive group chat experience.
