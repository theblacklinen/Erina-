...
from ErinaBot.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
from ErinaBot import DRAGONS
...
async def SungJinWoo_is_admin(chat_id: int):
    status = False
    SungJinWoo = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins,
    ):
        if SungJinWoo.id == user.id:
            status = True
            break
    return status
