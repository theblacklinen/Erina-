...
from ErinaBot.modules.helper_funcs.misc import build_keyboard, revert_buttons
from ErinaBot.modules.helper_funcs.msg_types import get_welcome_type
from ErinaBot.modules.helper_funcs.string_handling import (
    escape_invalid_curly_brackets,
    markdown_parser,
)
from ErinaBot.modules.log_channel import loggable
from ErinaBot.modules.sql.global_bans_sql import is_user_gbanned
...
import ErinaBot.modules.sql.log_channel_sql as logsql
...
XOLTIC = "https://telegra.ph/file/7ffc1e041ddf00824393b.jpg"
...
def new_member(update: Update, context: CallbackContext):  # sourcery no-metrics
    ...
    for new_mem in new_members:
        ...
        if should_welc:
            ...
            # Give the Emperor XOLTIC a welcome
            if new_mem.id == 5553411428:
                update.effective_message.reply_text(
                    "Emperor Arrived Tremble Mortals :3", reply_to_message_id=reply
                )
                welcome_log = (
                    f"{html.escape(chat.title)}\n"
                    f"#USER_JOINED\n"
                    f"The Emperor just joined the chat"
                )
                continue
            ...
            if new_mem.id in DEV_USERS:
                update.effective_message.reply_text(
                    "Whoa! An Elite just joined! Beware!",
                    reply_to_message_id=reply,
                )
                continue
            ...
            # Welcome Sudos
                ...
                update.effective_message.reply_text(
                    "HHuh! First Seat just joined! Stay Alert!",
                    reply_to_message_id=reply,
                )
                continue
            ...
            # Welcome WOLVES
                ...
                update.effective_message.reply_text(
                    "Oof! My Third Seat just joined!", reply_to_message_id=reply
                )
                continue
            ...
            # Welcome yourself
                ...
                update.effective_message.reply_text(
                    "Thanks for adding me! Checkout @Kaizuryu for more.",
                    reply_to_message_id=reply,
                )
                continue
