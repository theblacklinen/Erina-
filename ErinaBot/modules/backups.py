...
import ErinaBot.modules.sql.notes_sql as sql
from ErinaBot import dispatcher, LOGGER, OWNER_ID, JOIN_LOGGER, SUPPORT_CHAT
from ErinaBot.__main__ import DATA_IMPORT
from ErinaBot.modules.helper_funcs.chat_status import user_admin
from ErinaBot.modules.helper_funcs.alternate import typing_action

import ErinaBot.modules.sql.rules_sql as rulessql

import ErinaBot.modules.sql.blacklist_sql as blacklistsql
from ErinaBot.modules.sql import disable_sql as disabledsql

import ErinaBot.modules.sql.locks_sql as locksql
...
def export_data(update, context):
    ...
    with open("SungJinWoo_Robot{}Backup".format(chat_id), "w") as f:
    ...
    context.bot.sendDocument(
        current_chat_id,
        document=open("SungJinWoo_Robot{}Backup".format(chat_id), "rb"),
        caption="*Successfully Exported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This `SungJinWoo_Robot-Backup` was specially made for notes.".format(
            chat.title, chat_id, tgl,
        ),
        timeout=360,
    ...
    os.remove("SungJinWoo{}Backup".format(chat_id))  # Cleaning file
