...
from ErinaBot.modules.helper_funcs.chat_status import bot_admin, bot_can_delete
...
import ErinaBot.modules.sql.antilinkedchannel_sql as sql


@ErinaBotcmd(command="cleanlinked", group=112)
@bot_can_delete
...
@ErinaBotmsg(Filters.is_automatic_forward, group=111)
...
@ErinaBotcmd(command="antichannelpin", group=114)
@bot_admin
@user_admin(AdminPerms.CAN_RESTRICT_MEMBERS)
...
@ErinaBotmsg(Filters.is_automatic_forward | Filters.status_update.pinned_message, group=113)
def eliminate_linked_channel_msg(update: Update, _: CallbackContext):
    message = update.effective_message
    chat = update.effective_chat
