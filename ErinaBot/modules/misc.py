...
from ErinaBot.modules.disable import DisableAbleCommandHandler
import contextlib
import html
import time
...
from ErinaBot import DRAGONS as SUDO_USERS
from ErinaBot import DEMONS as SUPPORT_USERS
from ErinaBot import WOLVES as WHITELIST_USERS  
from ErinaBot import (
    dispatcher,
    OWNER_ID,
    DEV_USERS,
    TIGERS,
...
from ErinaBot.__main__ import STATS, USER_INFO
from ErinaBot.modules.helper_funcs.chat_status import user_admin, sudo_plus
from ErinaBot.modules.helper_funcs.extraction import extract_user
import ErinaBot.modules.sql.users_sql as sql
from ErinaBot.modules.users import __user_info__ as chat_count
from telegram import __version__ as ptbver
from psutil import cpu_percent, virtual_memory, disk_usage, boot_time
import datetime
import platform
from platform import python_version
from ErinaBot.modules.helper_funcs.decorators import ErinaBotcmd
...
@ErinaBotcmd(command='info', pass_args=True)
...
def get_user_info(chat: Chat, user: User) -> str:
    ...
    if user.id == OWNER_ID:        
        text += "\n\n<code>This User is the Emperor"
        disaster_level_present = True
    elif user.id in DEV_USERS:
        text += "\n\n<code>This user is a part of Elite Ten Council</code>"
        disaster_level_present = True
    elif user.id in SUDO_USERS:
        text += "\n\n<code>One of First Seats, touch him and you are dead meat</code>"
        disaster_level_present = True
    elif user.id in SUPPORT_USERS:
        text += "\n\n<code>This user is our Ally</code> "
        disaster_level_present = True
    elif user.id in TIGERS:
        text += "\n\n<code>One of my chef</code> :p"
        disaster_level_present = True
    elif user.id in WHITELIST_USERS:
        text += "\n\n<code>Member of ErinaBot Tech, totally cool right?</code>"
        disaster_level_present = True
    if disaster_level_present:
        text += ' [<a href="https://t.me/TheDestroyer/13">?</a>]'
    text += "\n"
...
@ErinaBotcmd(command='markdownhelp', filters=Filters.chat_type.private)
...
stats_str = '''
'''
@ErinaBotcmd(command='stats', can_disable=False)
@sudo_plus
def stats(update, context):
    uptime = datetime.datetime.fromtimestamp(boot_time()).strftime("%Y-%m-%d %H:%M:%S")
    botuptime = get_readable_time((time.time() - StartTime))
    status = "*╒═══「 Erina statistics: 」*\n\n"
    ...
    try:
        update.effective_message.reply_text(status +
            ...
            "\n\n[⍙ GitHub](https://github.com/theblacklinen/ErinaBot) | [Telegram](https://t.me/Erina_GroupBot)\n\n" +
            "╘══「 by [Xoltic](t.me/Xoltic) 」\n",
